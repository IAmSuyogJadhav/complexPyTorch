...
def conv3x3(in_planes, out_planes, stride=1, groups=1, dilation=1):
    ...
    return nn.ComplexConv2d(in_planes, out_planes, kernel_size=3, stride=stride,
                     padding=dilation, groups=groups, bias=False, dilation=dilation)


def conv1x1(in_planes, out_planes, stride=1):
    """1x1 convolution"""
    return nn.ComplexConv2d(in_planes, out_planes, kernel_size=1, stride=stride, bias=False)
...
class BasicBlock(Module):
    ...
    def forward(self, xr, xi):
        ...
        outr, outi = self.conv2(outr, outi)
        outr, outi = self.bn2(outr, outi)

        if self.downsample is not None:
            identityr, identityi = self.downsample(xr, xi)
...
class ResNet(Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        for m in self.modules():
            if isinstance(m, nn.ComplexConv2d):
                init.kaiming_normal_(m.conv_r.weight, mode='fan_out', nonlinearity='relu')
                init.kaiming_normal_(m.conv_i.weight, mode='fan_out', nonlinearity='relu')
            elif isinstance(m, (nn.ComplexBatchNorm2d)):  # , nn.GroupNorm)):  # ComplexGroupNorm not yet implemented
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.ComplexSequential(
        ...
        return nn.ComplexSequential(*layers)

    def _forward_impl(self, xr, xi):
        ...
        xr, xi = self.avgpool(xr, xi)
        xr, xi = torch.flatten(xr, 1), torch.flatten(xi, 1)
        xr, xi = self.fc(xr, xi)
...
#         model.load_state_dict(state_dict)
        pass  # Not using pretrained weights to avoid potential problems
    return model
